...
//Request - body = "" -> null
  Future<String> addRequest(Hydrant newHydrant, String userId) async {
    ...
    http.Response res =
        await http.post(Uri.parse(Uri.encodeFull("$_baseUrl/new/$userId")),
            headers: header,
            body: json.encode({
              "attacks": [
...
//bool
  Future<String> approveRequest(
    ...
    http.Response res = await http.post(
        Uri.parse(Uri.encodeFull("$_baseUrl/approve/$requestId&$userId")),
        headers: header,
        body: json.encode({
          "id": hydrant.getId(),
...
//bool
  Future<String> denyRequest(String requestId, String userId) async {
    Map<String, String> header = await BasicAuthConfig().getUserHeader();
    http.Response res = await http.post(
      Uri.parse(Uri.encodeFull("$_baseUrl/deny/$requestId&$userId")),
      headers: header,
    );
    return res.body;
  }
...
//List<Request>
  Future<String> getPendingRequestsByDistance(
    ...
    Map<String, String> header = await BasicAuthConfig().getUserHeader();
    http.Response res = await http.get(
      Uri.parse("$_baseUrl/pending/$latitude&$longitude&$distance"),
      headers: header,
    );
    return res.body;
  }
...
//List<Request>
  Future<String> getRequestsByDistance(
    ...
    Map<String, String> header = await BasicAuthConfig().getUserHeader();
    http.Response res = await http.get(
      Uri.parse("$_baseUrl/all/$latitude&$longitude&$distance"),
      headers: header,
    );
    return res.body;
  }
